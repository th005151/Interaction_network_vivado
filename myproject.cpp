...
#pragma HLS INTERFACE ap_vld port=N,E,receivers,senders,e
 ...
 if (!loaded_weights) {
  ...
  input_t influence[N_NODES_MAX][N_FEATURES];
  //#pragma HLS INTERFACE ap_vld port=effects,influence
  ...
  nnet::aggregation<input_t, input_t, graph_config1>(aggregation_tmp, aggregation);
...
//--------------------------------------------------------------------------- 
  ...
  #pragma HLS ARRAY_PARTITION variable=aggregation_rep complete dim=0
...
//--------------------------------------------------------------------------- 
  ...
  result_t e_tmp[4][N_EDGES_MAX/4][E_FEATURES];
  input_t q_tmp[4][N_NODES_MAX][E_FEATURES];
  #pragma HLS ARRAY_PARTITION variable=e_tmp complete dim=0
